...
- **The read is per-session, not bulk.** A route is not an independent record,
  ...
  `readRecord`. Hence a lazy per-run read, post-dedupe, like the HR series. It
  is reached only from inside `scanWatchSessions`, after that scan's
  availability + permission gate, so it inherits the "never touch the bridge
  unguarded" rule above rather than adding a new ungated entry point.
