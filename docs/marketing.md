...
Two static 1200×630 cards, both generated by `scripts/og-image/generate.mjs`
from `src/marketing/copy.json` (the single source of truth for the brand + hero
headline — `MarketingGate.tsx` imports the same file, so the card can't drift
from the page):

- `public/og-image.png` (`template.html`): site and app links. The hero headline
  beside the real home screen (`home-screen.png`, demo account).
- `public/og-watch.png` (`watch-template.html`): every `/watch/:token` link. One
  image for every runner, so its map is drawn, never a real place.

Regenerate locally with `npm run og:image` (needs a Chrome/Chromium binary;
found via `CHROME_BIN`, a Playwright chromium, or system paths). It drives Chrome
over the DevTools pipe rather than `--screenshot`, because new headless
subtracts invisible browser UI from `--window-size` and paints only ~543 of the
630 rows. Fonts are the committed Archivo files in `scripts/og-image/fonts/`,
never a network download, so a local render and CI's match. CI does it
automatically — `og-image.yml` re-renders and commits both PNGs on any change to
`copy.json` or `scripts/og-image/**` on a feature branch, so the refreshed cards
reach `main` with the copy change. Never hand-edit the committed PNGs.

Chat apps never run JavaScript, so a run link needs its own static page to
preview as a run: `npm run build` ends with `scripts/watch-page.mjs`, which writes
