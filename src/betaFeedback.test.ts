...
describe("submitBetaFeedback", () => {
  ...
  it("writes the context the maintainer reads in the SQL editor", async () => {
    await submitBetaFeedback({ body: "charts unreadable", source: "coach", inputMode: "voice" });
  ...
  it("trims, and stays inside the column's length check", async () => {
    ...
    await submitBetaFeedback({ body: `  ${"x".repeat(MAX_FEEDBACK_LEN + 500)}  `, source: "dash", inputMode: "text" });
    expect(row().body.length).toBe(MAX_FEEDBACK_LEN);
  });

  it("refuses an empty report and one from a signed-out client", async () => {
    await expect(submitBetaFeedback({ body: "   ", source: "dash", inputMode: "text" }))
      .rejects.toThrow();
    h.userId.mockReturnValue(null);
    await expect(submitBetaFeedback({ body: "real", source: "dash", inputMode: "text" }))
      .rejects.toThrow();
  ...
  it("does not send the maintainer an email for a write that failed", async () => {
    ...
    await expect(submitBetaFeedback({ body: "real", source: "settings", inputMode: "text" }))
      .rejects.toBeTruthy();
    expect(h.notify).not.toHaveBeenCalled();
  });
});
